...
for platform in "${!media_status[@]}"; do
  if [[ "${media_status[$platform]}" != "Yes" ]]; then
    ...
    #将platform存入routes生成配置文件时读取
    if [[ -z "${routes[$best_alias]}" ]]; then
        routes[$best_alias]+="# $platform"
    else
        routes[$best_alias]+="^# $platform"
    fi
		#将rules_list存入routes生成配置文件时读取
...
for alias in $(echo "$NODES_JSON" | jq -r 'keys[]'); do
  ...
  unset IFS # 重置 IFS 回到默认
  echo -e ']' >> "$routes_file"

  # 获取节点信息并写入出口节点
  echo -e "\n#出口 $alias\n[[routes.Outs]]\ntype=\"$(echo "$NODES_JSON" | jq -r --arg alias "$alias" '.[$alias].type // empty')\"\nserver=\"$(echo "$NODES_JSON" | jq -r --arg alias "$alias" '.[$alias].domain // empty')\"\nport=$(echo "$NODES_JSON" | jq -r --arg alias "$alias" '.[$alias].port // empty')\npassword=\"$(echo "$NODES_JSON" | jq -r --arg alias "$alias" '.[$alias].uuid // empty')\"\ncipher=\"$(echo "$NODES_JSON" | jq -r --arg alias "$alias" '.[$alias].cipher // empty')\"" >> "$routes_file"
done
